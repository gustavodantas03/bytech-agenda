...
def main():
    if len(sys.argv)<3:
        ...
        raise SystemExit(2)
    empresa_id=int(sys.argv[1]); publica=sys.argv[2].rstrip('/')
    empresa_id = int(sys.argv[1])
publica = sys.argv[2].rstrip("/")

# Permite URLs locais durante o desenvolvimento
desenvolvimento = (
...
# Em produção continua exigindo HTTPS
if not desenvolvimento and not publica.startswith("https://"):
    raise SystemExit(
        "A URL pública do webhook deve começar com https://"
    )
    ...
    if not config: raise SystemExit("Não foi possível criar a configuração WhatsApp da empresa.")
    base,key,_=obter_configuracao_global(); token=os.getenv("EVOLUTION_WEBHOOK_TOKEN","").strip()
    url=f"{publica}/api/webhooks/evolution/{config['instance_name']}" + (f"?token={token}" if token else "")
    payload={"webhook":{"enabled":True,"url":url,"webhookByEvents":False,"webhookBase64":False,"events":["MESSAGES_UPSERT"]}}
    req=request.Request(f"{base}/webhook/set/{config['instance_name']}",data=json.dumps(payload).encode(),headers={"apikey":key,"Content-Type":"application/json"},method="POST")
    try:
        with request.urlopen(req,timeout=20) as resp: print(resp.read().decode())
    except error.HTTPError as exc:
        print(exc.read().decode()); raise
