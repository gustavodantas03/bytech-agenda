...
## 11. Ajustes de interface e conexão do WhatsApp sem computador

**Botão "Precisa de ajuda?" perto de "Sair do painel":** removido. Era só
um cartão decorativo sem nenhum link ou ação — não fazia nada quando
clicado, então tirei.

**Espaçamento do login (admin e master):** o botão "Entrar" estava
colado no campo de senha. Ajustado o espaçamento entre os campos e o
botão nas duas telas de login.

**Checkboxes de recursos por plano (painel master):** reduzidos —
estavam com padding e fonte grandes demais para uma lista de opções.

**Conectar WhatsApp sem computador (o mais importante):** até agora só
existia QR Code, o que não funciona quando o estabelecimento só tem UM
celular (o mesmo que vai ficar com o WhatsApp) — não dá pra usar a câmera
de um aparelho pra escanear a tela do próprio aparelho. Adicionei uma
segunda forma de conectar: **código de pareamento**. O dono digita o
número de WhatsApp da empresa no painel, o sistema gera um código de 8
dígitos, e a pessoa digita esse código direto no WhatsApp do celular
(Aparelhos conectados → Conectar com número de telefone) — sem precisar
de câmera nem de outro aparelho.

**Importante ficar de olho:** esse recurso (`pairingCode`) é documentado
oficialmente pela Evolution API, mas pesquisando relatos de outros
usuários encontrei vários casos de instabilidade — código gerado mas
rejeitado pelo WhatsApp, ou conectando e caindo pouco depois. Pode
funcionar direto ou pode precisar de mais de uma tentativa dependendo da
versão da Evolution API rodando na sua VPS. Recomendo testar com um
número real assim que a Evolution API estiver no ar, antes de divulgar
essa opção para os clientes. Se der problema recorrente, o QR Code
continua sendo a opção mais confiável — nesse caso, oriente o
estabelecimento a pedir emprestado qualquer outro aparelho por 30
segundos (celular de um funcionário, tablet) só para exibir a tela do QR
Code, já que isso não exige instalar nada.

Testado: migração da nova coluna no banco (nova instalação e banco já
existente), renderização da nova tela com as duas opções de conexão, e a
rota de solicitação de código respondendo sem quebrar mesmo sem uma
Evolution API real disponível no ambiente de teste.

## 12. Overflow horizontal no mobile + suporte funcional

**Overflow horizontal (a causa da faixa cinza sobrando na tela do
celular):** identificado pelos prints que você mandou. O menu lateral
(admin e master) fica escondido fora da tela via `transform` quando o
menu está fechado no mobile, mas nada impedia esse espaço de continuar
"contando" na largura da página — por isso sobrava aquela faixa cinza à
direita e o conteúdo real ficava espremido à esquerda, cortado. Corrigido
adicionando `overflow-x: hidden` no `html` e `body` do sistema inteiro —
essa é a causa raiz de praticamente todo o problema de responsividade
mobile que você reportou, então deve ter resolvido a maior parte do que
estava incomodando.

**Botão "Bytech Agenda" embaixo de "Sair do painel":** removido — era só
um texto de versão instalada, sem nenhuma função, mesmo mostrando "Bytech"
no nome (por isso a confusão da vez passada). Não confundir com o
"Precisa de ajuda?", que era outro elemento.

**"Precisa de ajuda?" agora funciona de verdade:** antes era decorativo,
agora abre um formulário de contato de verdade
(`/admin/suporte`). O cliente manda a mensagem, ela cai direto na sua
caixa de suporte no painel master (`/master/suporte`, com contador de
pendentes no menu), você responde por lá, e a resposta aparece
automaticamente no histórico do cliente. Não precisa configurar e-mail
nem nada externo — fica tudo dentro do próprio sistema.

Testado: migração da nova tabela (banco novo e banco já existente), o
fluxo completo de ida e volta (cliente manda → aparece pro master →
master responde → cliente vê a resposta), e confirmei que o CSS de
overflow está sendo servido corretamente.
